  ...
  const generateImage = async () => {
    ...
    const singleGenerate = async (seed: number): Promise<string | null> => {
        ...
        try {
            ...
            try {
                // Attempt 2: Flash Model with Aspect Ratio
                ...
                try {
                    // Attempt 3: Flash Model Raw (No Config) - Last Resort
                    console.warn("Attempting raw generation...");
                    return await tryGenerate('gemini-2.5-flash-image', {});
                } catch(finalErr) {
                    return null;
                }
    ...
    try {
        ...
        if (validImages.length > 0) {
            ...
            throw new Error("GENERATION_ABORTED: ALL_ATTEMPTS_FAILED");
        }
            ...
            <div className="flex flex-col gap-4 border-t border-dim pt-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(["1:1", "16:9", "9:16", "4:3", "3:4", "2:3", "21:9"] as AspectRatio[]).map((ratio) => (
                      <button
                        key={ratio}
